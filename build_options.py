...
scalar_options=[
("Cosmological parameters",scalar_options_set_1)
]
...
def generate_options_type(options):
	S=StringIO.StringIO()
	S.write("""/* DO NOT EDIT THIS FILE - IT IS GENERATED FROM build_options.py. Edit that. */
...
def generate_default_parameter_file(options):
	S=StringIO.StringIO()
...
file(C_HANDLER_SOURCE,"w").write(generate_handler(scalar_options))
file(C_OPTIONS_HEADER,"w").write(generate_options_type(scalar_options))
file(DEFAULT_OPTIONS_FILENAME,"w").write(generate_default_parameter_file(scalar_options))
